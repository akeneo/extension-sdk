...
function App() {
  ...
  const fetchProductAndOrders = async () => {
      if (!shopifyUrl) {
        setError('No Shopify store URL configured. Please configure the \'shopify_url\' custom variable in your extension configuration.');
        return;
      }

      const storeUrl = shopifyUrl;
      try {
        setIsLoading(true);
        setError(null);
          ...
          {isLoadingProduct ? (
            ...
            <div style={{
              ...
              <div style={{
                ...
                <button
                  onClick={() => {
                    globalThis.PIM.navigate.external(`${shopifyUrl}/admin/products/${shopifyProduct.id}`);
                  }}
                  style={{
                    display: 'inline-flex',
                    alignItems: 'center',
                  ...
                  onMouseEnter={(e) => {
                    e.currentTarget.style.backgroundColor = '#4A3A9E';
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.backgroundColor = '#5E4ABA';
                  }}
                >
                  View in Shopify
                </button>
