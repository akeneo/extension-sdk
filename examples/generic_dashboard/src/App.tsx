...
const getProductName = (values: { [key: string]: Array<{ locale: string | null; data: string; }> } | undefined): string => {
    ...
    if (nameValues && nameValues.length > 0) {
      for (const locale of localePriority) {
        ...
        if (nameValue && nameValue.data) {
          return nameValue.data; // Found a name in a priority locale
        }
      }
      // If no priority locale found for this attribute, try the first available one
  ...
  return 'N/A'; // No name found in any of the priority attributes
};
...
function App() {
  ...
  const [productStatusData, setProductStatusData] = useState({ labels: [], datasets: [] });
  const [pricingStatusData, setPricingStatusData] = useState({ labels: [], datasets: [] });
